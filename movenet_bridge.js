(function () {
  ...
  async function init(containerId) {
    ...
    state.initPromise = (async () => {
      ...
      // Use cover so the camera fills the entire container
      ...
      const loop = async (ts) => {
        ...
        // NOTE: overlayEnabled now only controls whether we DRAW the
        // skeleton/keypoints on the canvas. Pose estimation and the
        // keypointCallback (confidence, calories, etc.) keep running
        // regardless, so toggling the overlay off no longer breaks tracking.
        ...
        try {
          ...
          if (poses && poses.length) {
              ...
              try {
                const normalizedKp = kp.map((k, idx) => {
                  ...
                  return {
                    ...
                    x: (k.x ?? 0) / vw,
                    y: (k.y ?? 0) / vh,
                    confidence: score,  // Pass actual confidence from MoveNet (0-1)
                  };
                });
            ...
            // Only draw the skeleton and keypoints if the overlay is enabled.
